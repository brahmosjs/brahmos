...
function updateArrayNodes(part, nodes, oldNodes = [], context, isSvgPart) {
  ...
  for (let i = 0; i < nodesLength; i++) {
    ...
    const currentOldNode = spliceUnusedNodes(
      i,
      oldNodes,
      parentNode,
      lastChild,
    ...
    const forceUpdate =
      (isPrimitiveNode(node) ||
        (node && currentOldNode && node.key !== currentOldNode.key)) &&
      'all';
...
function updateTagNode(part, node, oldNode, context, forceUpdate, isSvgPart) {
  ...
  if (freshRender) {
     ...
     */
    templateNode.nodes = insertBefore(
      parentNode,
      nextSibling,
  ...
  const onCorrectPos =
    firstChild && firstChild.previousSibling === previousSibling;
