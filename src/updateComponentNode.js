...
function renderWithErrorBoundaries(
	part,
	node,
	context,
	shouldUpdate,
	...
	const { type: Component, nodeType, componentInstance, props } = node;
	...
	try {
		 ...
		 */
		componentInstance.__lastNode = updateNode(
			part,
			newNodes,
			oldNodes,
			context,
		...
		if (isClassComponent && handleError) {
			...
			const errorState = callLifeCycle(Component, 'getDerivedStateFromError', [
				props,
				state,
			]);
 ...
 */
export default function updateComponentNode(
	part,
	node,
	oldNode,
	context,
	...
	if (isClassComponent) {
		...
		if (componentInstance instanceof PureComponent && checkShouldUpdate) {
			shouldUpdate =
				!shallowEqual(state, prevState) || !shallowEqual(props, prevProps);
		}
		...
		if (shouldComponentUpdate && shouldUpdate && checkShouldUpdate) {
			shouldUpdate = shouldComponentUpdate.call(
				componentInstance,
				props,
				state,
			);
	...
	if (shouldUpdate || forceUpdate === 'all') {
		...
		snapshot = callLifeCycle(componentInstance, 'getSnapshotBeforeUpdate', [
			prevProps,
			prevState,
		]);
		...
		if (isFirstRender) {
			...
			if (isClassComponent) {
				...
				callLifeCycle(componentInstance, 'componentDidUpdate', [
					prevProps,
					prevState,
					snapshot,
				]);
