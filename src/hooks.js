...
export function useMemo(create, dependencies) {
	...
	const shouldUpdate = hook =>
		isDependenciesChanged(dependencies, hook.dependencies);
...
function useEffectBase(effectHandler, dependencies) {
	...
	const hook = {
		...lastHook,
		isDependenciesChanged: isDependenciesChanged(
			dependencies,
			lastHook.dependencies,
		),
		dependencies,
		effect() {
